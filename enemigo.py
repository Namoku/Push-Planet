...
class Monster(pygame.sprite.Sprite):
    def __init__(self, position):
        self.bobby = pygame.Rect(level1.enemigoPosX, level1.enemigoPosY, 39, 40)
    ...
    def move(self, direction, speedx, speedy):
        ...
        if direction == 'left':
            self.clip(self.down_states)
            self.move_single_axis(-speedx, 0)
        if self.bobby.colliderect(level1.player.bobby):
            if speedx < 0:
                self.bobby.left =level1. player.bobby.right + 40
                level1.vida -= 50
            if speedx > 0:
                self.bobby.right = level1.player.bobby.left - 40
                level1.vida -= 50
            if speedy < 0:
                self.bobby.top = level1.player.bobby.bottom + 40
                level1.vida -= 50
            if speedy > 0:
                self.bobby.bottom = level1.player.bobby.top - 40
                level1.vida -= 50
