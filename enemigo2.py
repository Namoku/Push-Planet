...
class Monster(pygame.sprite.Sprite):
    def __init__(self, position):
        self.bobby = pygame.Rect(level2.enemigoPosX, level2.enemigoPosY, 39, 40)
    ...
    def move(self, direction, speedx, speedy):
        ...
        if direction == 'left':
            self.clip(self.down_states)
            self.move_single_axis(-speedx, 0)
        if self.bobby.colliderect(level2.player.bobby):
            if speedx < 0:
                self.bobby.left =level2. player.bobby.right + 40
                level2.vida -= 60
            if speedx > 0:
                self.bobby.right = level2.player.bobby.left - 40
                level2.vida -= 60
            if speedy < 0:
                self.bobby.top = level2.player.bobby.bottom + 40
                level2.vida -= 60
            if speedy > 0:
                self.bobby.bottom = level2.player.bobby.top - 40
                level2.vida -= 60
